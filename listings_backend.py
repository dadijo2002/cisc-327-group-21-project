...
"""
This is an incomplete sample implementation of some of the backend features
 for the listings class
Group 21 - CISC 327
...
class Listings(db.Model):  # SQLified the old Listings class a little bit
    ...
    def title_check(self):
        ...
        if self.title.isalnum() and (self.title.find(" ") == 0 or
                                     self.title.find(" ") ==
                                     self.username.length) and len(
                self.title) <= 80:
            return True
        else:
            return False
